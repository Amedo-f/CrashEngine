...
namespace Twinsanity.TwinsanityInterchange.Implementations.PS2.Items.RM2.Code.AgentLab
{
    public class PS2BehaviourCommand : ITwinBehaviourCommand
    {
        ...
        public void WriteText(StreamWriter writer, Int32 tabs = 0)
        {
            ...
            // ReadText requires an "@{Version} " prefix (asserts on it) that this never actually
            // wrote — WriteText/ReadText were not a working round-trip before this fix. Also,
            // for any CommandIndex not in defs.CommandMap, MapCommand returns a synthetic
            // "AUnknown_N" label that ReadText's name-lookup branch can never resolve back
            // (CommandMap only ever contains REAL names) — use the "ById_N" explicit form
            // ReadText already has a fast path for instead, exactly like it does when written
            // that way manually.
            string header = defs.CommandMap.ContainsKey(CommandIndex.ToString())
                ? MapCommand(CommandIndex, defs)
                : $"ById_{CommandIndex}";
            StringUtils.WriteTabulated(writer, $"@{Version} {header}(", tabs);
